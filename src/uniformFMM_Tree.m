    ...
    methods
        ...
        function obj = uniformFMM_Tree(source_points, source_charges, source_order, ...
            ...
            arguments
                ...
                level(1, 1) = 0;
                order(1, 1) = 0;
                % The initial box must be square.
                center(1, 2) = [0.5, 0.5];
                half_length(1, 1) = 0.5;
            end
            ...
            if obj.level_ == 0
                obj = BuildTree(obj, ceil(log2(length(obj.source_charges_)) / 2));
                obj = SetList(obj);
            end
        end
        ...
        function obj = FMM_Alg(obj, tol)
            ...
            for tmplevel = obj.num_level_ : -1 : 0
                obj = RecursiveM2M(obj, p, tmplevel);
            end
        ...
        function obj = RecursiveM2M(obj, p ,whatlevel)
            % RecursiveM2M Multipole to multipole recursively.
            
            if obj.level_ == whatlevel
                obj = M2M(obj, p);
            else
                for iter = 1 : 4
                    obj.children_{iter} = RecursiveM2M(obj.children_{iter}, p, whatlevel);
                end
            end
        end
        ...
        function obj = MultipoleExpansion(obj, p)
            % MultipoleExpansion Compute multipole expansion.
            % \phi(z) \approx a_0 \log(z) + \sum_{k = 1}^p a_k / z^k
            % where {a_k}_{k = 0}^p is stored in multipole_expansion_.
...
%             z = complex(5 * rand + 5, 5 * rand + 5);
%             zex = 0;
%             for i = 1 : m
%                 zi = complex(obj.source_points_(i, 1), obj.source_points_(i, 2));
%                 zex = zex + obj.source_charges_(i) * log(z - zi);  
%             end
%             zcomp = obj.multipole_expansion_(1) * log (z - zc);
%             for k = 1 : p
%                 zcomp = zcomp + obj.multipole_expansion_(k + 1) / (z - zc)^k;
%             end
%             disp(abs(zex - zcomp));
        end
        
        function obj = RecursiveM2L2L(obj, p ,whatlevel)
            % RecursiveM2L2L Multipole to local recursively.
        ...
        function potential = L2T(obj, potential)
            % L2T Compute the potential of target points.
            ...
            if obj.leaf_ == 1
                ...
                for iter = 1 : 4
                    potential = L2T(obj.children_{iter}, potential);
                end
            end
        end

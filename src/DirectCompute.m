...
% DirectCompute Compute potential from source points to target points directly. It is given by the
% function log (1 / |x - y|)
...
source_points_complex = complex(source_points(:, 1), source_points(:, 2));
target_points_complex = complex(target_points(:, 1), target_points(:, 2));
potential = -log(abs(target_points_complex - source_points_complex.')) * source_charges;
